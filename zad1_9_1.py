...
class Blocks(Problem):
    ...
    def click(self, x, y, state):
        state1 = list(list(d) for d in state)
        states = [(x, y), (x + 1, y), (x - 1, y), (x, y - 1), (x, y + 1)]
        for i, j in states:
            if self.isValid(i, j):
                color = state[i][j]
                c = color[2]
                state1[i][j] = (i, j, self.value1(c))
        state2 = tuple(tuple(row) for row in state1)
        return state2
    ...
    def successor(self, state):
        ...
        for i in range(self.n):
            for j in range(self.n):
                neighbours[f'x: {i}, y: {j}'] = self.click(i, j, state)
        return neighbours
...
if __name__ == "__main__":
    n1 = int(input())
    matrica = input().split(',')
    matrica1 = [[0 for i in range(n1)] for j in range(n1)]
    k = 0
    for i in range(n1):
        for j in range(n1):
            matrica1[i][j] = (i, j, int(matrica[k]))
            k += 1
    matrica2 = tuple(tuple(row) for row in matrica1)
    problem = Blocks(matrica2, n1)
