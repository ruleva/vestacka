...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            state1 = list(state)
            state1[i] = (x, y + 1)
            if self.check_valid(state1):
                succ[f'Pomesti kvadratche {i + 1} gore'] = tuple(state1)
            state1[i] = (x, y - 1)
            if self.check_valid(state1):
                succ[f'Pomesti kvadratche {i + 1} dolu'] = tuple(state1)
            state1[i] = (x - 1, y)
            if self.check_valid(state1):
                succ[f'Pomesti kvadratche {i + 1} levo'] = tuple(state1)
            state1[i] = (x + 1, y)
            if self.check_valid(state1):
                succ[f'Pomesti kvadratche {i + 1} desno'] = tuple(state1)
