...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        x, y = state
        if state not in walls and (0 <= x < n and 0 <= y < n):
            return True
        else:
            return False
    ...
    def successor(self, state):
        ...
        for i in range(3):
            state1 = (x, y + i+1)
            if self.check_valid(state1, self.walls, self.n):
                successors[f'Gore {i+1}'] = state1
        for i in range(3):
            state1 = (x + i+1, y)
            if self.check_valid(state1, self.walls, self.n):
                successors[f'Desno {i+1}'] = state1
        #print(successors)
        return successors
    ...
    def h(self, node):
        state = node.state
        x, y = state
        return (abs(x - goal_pos[0]) + abs(y - goal_pos[1])) / 3
