...
class Football(Problem):
    def __init__(self, initial, oponents, goal):
    ...
    @staticmethod
    def check_valid(state, oponents):
        man_pos = state[0]
        ball_pos = state[1]
        return 0 <= man_pos[0] < 8 and \
            0 <= man_pos[1] < 6 and \
            0 <= ball_pos[0] < 8 and \
            0 <= ball_pos[1] < 6 and man_pos not in oponents and ball_pos not in oponents

    def ball_not_allowed(self):
        pos = []
        pos += self.oponents
        for opponent in self.oponents:
            x, y = opponent
            pos += [(x + 1, y), (x - 1, y), (x + 1, y + 1), (x - 1, y - 1), (x - 1, y + 1), (x + 1, y - 1), (x, y + 1),
                    (x, y - 1)]
        return pos
    ...
    def successor(self, state):
        ...
        mx, my = man
        bx, by = ball
        #print(self.ball_not_allowed())
        if self.check_valid(((mx, my + 1), (bx, by)), self.oponents):
                successors['Pomesti coveche gore'] = ((mx, my + 1), (bx, by))
        if self.check_valid(((mx, my - 1), (bx, by)), self.oponents):
                successors['Pomesti coveche dolu'] = ((mx, my - 1), (bx, by))
        if self.check_valid(((mx + 1, my), (bx, by)), self.oponents):
                successors['Pomesti coveche desno'] = ((mx + 1, my), (bx, by))
        if self.check_valid(((mx + 1, my + 1), (bx, by)), self.oponents):
                successors['Pomesti coveche gore-desno'] = ((mx + 1, my + 1), (bx, by))
        if self.check_valid(((mx + 1, my - 1), (bx, by)), self.oponents):
                successors['Pomesti coveche dolu-desno'] = ((mx + 1, my - 1), (bx, by))
        if self.check_valid(((mx, my + 1), (bx, by + 1)), self.oponents) and (bx, by + 1) not in self.ball_not_allowed() and (
                mx, my + 1) == (bx, by):
            successors['Turni topka gore'] = ((mx, my + 1), (bx, by + 1))
        if self.check_valid(((mx, my - 1), (bx, by - 1)), self.oponents) and (bx, by - 1) not in self.ball_not_allowed() and (
                mx, my - 1) == (bx, by):
            successors['Turni topka dolu'] = ((mx, my - 1), (bx, by - 1))
        if self.check_valid(((mx + 1, my), (bx + 1, by)), self.oponents) and (bx + 1, by) not in self.ball_not_allowed() and (
                mx + 1, my) == (bx, by):
            successors['Turni topka desno'] = ((mx + 1, my), (bx + 1, by))
        if self.check_valid(((mx + 1, my + 1), (bx + 1, by + 1)),
                              self.oponents) and (bx + 1, by + 1) not in self.ball_not_allowed() and (mx + 1, my + 1) == (bx, by):
            successors['Turni topka gore-desno'] = ((mx + 1, my + 1), (bx + 1, by + 1))
        if self.check_valid(((mx + 1, my - 1), (bx + 1, by - 1)),
                              self.oponents) and (bx + 1, by - 1) not in self.ball_not_allowed() and (mx + 1, my - 1) == (bx, by):
            successors['Turni topka dolu-desno'] = ((mx + 1, my - 1), (bx + 1, by - 1))
...
if __name__ == '__main__':
    ...
    goal = ((7, 2), (7, 3))
    oponents = ((3, 3), (5, 4))
    problem = Football((man_pos, ball_pos), oponents, goal)
