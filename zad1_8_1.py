...
class Game(Problem):
    def __init__(self, N, initial, blacks, goal):
    ...
    def is_available(self, state):
        x, y = state
        if 0 <= x < self.N and 0 <= y < self.N and state not in self.blacks:
            return True
    ...
    def successor(self, state):
        ...
        for tup in state:
            x, y = tup
            state1 = (x - 2, y + 2)
            if self.is_available(state1) and (x - 1, y + 1) in state:
                list1 = list(state)
                list1.remove(tup)
                list1.remove((x - 1, y + 1))
                list1.append(state1)
                neighbours[f'Gore Levo: (x={x},y={y})'] = tuple(list1)
            state1 = (x + 2, y + 2)
            if self.is_available(state1) and (x + 1, y + 1) in state:
                list1 = list(state)
                list1.remove(tup)
                list1.remove((x + 1, y + 1))
                list1.append(state1)
                neighbours[f'Gore Desno: (x={x},y={y})'] = tuple(list1)
            state1 = (x - 2, y - 2)
            if self.is_available(state1) and (x - 1, y - 1) in state:
                list1 = list(state)
                list1.remove(tup)
                list1.remove((x - 1, y - 1))
                list1.append(state1)
                neighbours[f'Dolu Levo: (x={x},y={y})'] = tuple(list1)
            state1 = (x + 2, y - 2)
            if self.is_available(state1) and (x + 1, y - 1) in state:
                list1 = list(state)
                list1.remove(tup)
                list1.remove((x + 1, y - 1))
                list1.append(state1)
                neighbours[f'Dolu Desno: (x={x},y={y})'] = tuple(list1)
            state1 = (x - 2, y)
            if self.is_available(state1) and (x - 1, y) in state:
                list1 = list(state)
                list1.remove(tup)
                list1.remove((x - 1, y))
                list1.append(state1)
                neighbours[f'Levo: (x={x},y={y})'] = tuple(list1)
            state1 = (x + 2, y)
            if self.is_available(state1) and (x + 1, y) in state:
                list1 = list(state)
                list1.remove(tup)
                list1.remove((x + 1, y))
                list1.append(state1)
                neighbours[f'Desno: (x={x},y={y})'] = tuple(list1)
        return neighbours
    ...
    def goal_test(self, state):
        ...
        if len(state) != 1:
            return False
        #print(state[0],self.goal)
        return state[0] == self.goal


if __name__ == "__main__":
    n = int(input())
    brojNaTopcinja = int(input())
    topcinja = []
    for i in range(brojNaTopcinja):
        topcinja.append(tuple(map(int, input().split(","))))
    brojNaCrni = int(input())
    crni = []
    for i in range(brojNaCrni):
        crni.append(tuple(map(int, input().split(","))))
    gol = (int(n / 2) , n - 1)
    problem = Game(n, tuple(topcinja), crni, gol)
