...
class Block(Problem):
    ...
    def move(self, state, x, y):
        ...
        l_b2 = ''
        if len(last_block) != 0:
            l_b = last_block[len(last_block) - 1]
        last_block2 = state[y]
        if len(last_block2) is not 0:
            l_b2 = last_block2[len(last_block2) - 1]
        list_ = list(last_block2)
        list_1 = list(last_block)
        if l_b != '':
            if l_b2 != '' and l_b <= l_b2:
                list_.append(l_b)
                list_1.pop(len(last_block) - 1)
            elif l_b2 == '':
                list_.append(l_b)
                list_1.pop(len(last_block) - 1)
        list1 = list(state)
        list1[y] = list_
        list1[x] = list_1
        # print(tuple(tuple(f) for f in list1))
        return tuple(tuple(f) for f in list1)

    def cut_blanks(self, state):
        new_tupleoftuples = []
        for tuple1 in state:
            new_tuple = []
            for i in tuple1:
                if i != '':
                    new_tuple.append(i)
            new_tupleoftuples.append(tuple(new_tuple))
        return tuple(s for s in new_tupleoftuples)
    ...
    def successor(self, state):
        ...
        for i in range(size):
            for j in range(size):
                if (i != j):
                    neighbours[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = self.move(state, i, j)
    ...
    def goal_test(self, state):
        state1 = self.cut_blanks(state)
        goal1=self.cut_blanks(self.goal)
        return state1 == goal1


if __name__ == "__main__":
    init = tuple(input().split(';'))
    f = []
    for tup in init:
        tupl = tup.split(',')
        f.append(tupl)
    tup = tuple(tuple(d) for d in f)
    goal1 = tuple(input().split(';'))
    g = []
    for go in goal1:
        tupl = go.split(',')
        g.append(tupl)
    goal2 = tuple(tuple(d) for d in g)
    problem = Block(tup, goal2)
