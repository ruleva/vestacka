...
class BlackWhite(Problem):
    ...
    def check_valid(self, state1):
        x, y = state1
        return 0 <= x < self.n and 0 <= y < self.n

    def to_tuple(self, l):
        return tuple(tuple(le) for le in l)
    ...
    def successor(self, state):
        ...
        for i in range(self.n):
            for j in range(self.n):
                state1 = list(list(s) for s in state)
                state1[i][j] = self.mirror(state1[i][j])
                for k in range(-1, 2):
                    if self.check_valid((i + k, j)):
                        state1[i + k][j] = self.mirror(state1[i + k][j])
                    if self.check_valid((i, j + k)):
                        state1[i][j + k] = self.mirror(state1[i][j + k])

                successors[f'x: {i}, y: {j}'] = self.to_tuple(state1)
...
if __name__ == "__main__":
    N = int(input())
    black_white = input().split(",")
    init = []
    k = 0
    for i in range(N):
        ii = []
        for j in range(N):
            ii.append(black_white[k])
            k += 1
        init.append(tuple(ii))
    problem = BlackWhite(tuple(init), N)
